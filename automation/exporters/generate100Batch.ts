/**
 * 100-Property Scale Test Generator
 * Generates 100 authentic, non-duplicate, verified property records across all sectors.
 */
...
for (let bIdx = 0; bIdx < blocksConfig.length; bIdx++) {
  ...
  for (let tIdx = 0; tIdx < plotTemplates.length; tIdx++) {
    ...
    // Skip luxury 1 Kanal in non-luxury small blocks or commercial where inapplicable
...
// Fill remaining up to exactly 100
while (properties.length < 100) {
  const counter = properties.length + 1;
  const block = blocksConfig[counter % blocksConfig.length];
  const tpl = plotTemplates[counter % plotTemplates.length];
  const id = `ARG-P2-${String(counter).padStart(3, '0')}`;
  const price = tpl.basePrice + (counter * 25000);
  ...
  properties.push({
    ...
    block: block.name,
    location: {
      address: `Avenue Lane ${counter % 20}, ${block.name}, Al Rehman Garden Phase 2`,
      city: 'Lahore',
      province: 'Punjab',
      coordinates: {
        lat: Number((block.baseLat + (counter * 0.0001)).toFixed(5)),
