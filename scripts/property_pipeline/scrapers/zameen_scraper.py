"""Robots-aware Zameen public listing adapter."""
...
PROPERTY_LINK = re.compile(r"zameen\.com/(?:Property|property)/", re.I)


def scrape(config: PipelineConfig = CONFIG) -> ScrapeResult:
    config.ensure_directories()
    result = ScrapeResult(source="Zameen")
    if not config.zameen_enabled:
        result.status = "disabled"
        result.message = "Zameen scraper disabled by ZAMEEN_SCRAPER_ENABLED."
        return result
    ...
    while queue and search_pages_visited < config.max_pages:
        ...
        for listing_url in discover_links(html, page_url, PROPERTY_LINK):
            ...
            try:
                ...
                result.pages_visited += 1
                listing = parse_listing_page("Zameen", listing_url, detail_html)
            except ComplianceError as exc:
    ...
    result.message = f"Collected {len(result.listings)} unique public records."
    return result


def main() -> int:
    result = scrape()
    destination = CONFIG.staging_dir / "raw-zameen.json"
    write_json(destination, result.to_dict())
    print(f"Zameen: {result.status}; {len(result.listings)} listings; {result.pages_visited} pages visited.")
